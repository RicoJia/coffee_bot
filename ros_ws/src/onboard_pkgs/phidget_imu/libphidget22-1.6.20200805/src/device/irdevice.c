...
//#define USE_SENDRAW_TO_TEST_DECODE
...
PhidgetIRDevice_bridgeInput(PhidgetChannelHandle ch, BridgePacket *bp) {
	...
	case BP_TRANSMITRAW:
#ifdef USE_SENDRAW_TO_TEST_DECODE
	{
		int i, dataLength;
		mostime_t duration;
		uint32_t us;
		const uint32_t *ubuffer;

		ubuffer = getBridgePacketUInt32Array(bp, 0);
		dataLength = getBridgePacketArrayCnt(bp, 0);
...
sendRAWData(mosiop_t iop, PhidgetIRDeviceHandle phid, int *data, int length, int carrier, double dutyCycle, int gap) {
	...
	case PHIDUID_1055_1:
		...
		if ((result = PhidgetDevice_transferpacket(iop, (PhidgetDeviceHandle)phid, PHIDGETUSB_REQ_DEVICE_WRITE, 0, 0, buffer, &len, 5000)) != EPHIDGET_OK) {
			return result;
		}
