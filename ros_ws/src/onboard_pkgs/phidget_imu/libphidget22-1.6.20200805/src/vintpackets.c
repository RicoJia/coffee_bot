...
static PhidgetReturnCode
sendVINTMOIST(PhidgetChannelHandle ch, BridgePacket *bp) {
	uint8_t buf[4];
...
static PhidgetReturnCode
recvVINTMOIST(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {
	double value;
	int pkt;
...
recvADP1001(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {
	...
	case PHIDCHUID_ADP1001_DATAADAPTER_100:
		...
		case VINT_PACKET_TYPE_SPI_DATAIN:
			return (bridgeSendToChannel(ch, BP_DATAIN, "%*R%u", (int)(len - 1), buf, overrun));
		default:
			MOS_PANIC("Unexpected packet type");
		}
		break;
